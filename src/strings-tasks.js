...
function removeFirstOccurrences(str, value) {
  const j = str.indexOf(value);
  const jEnd = j + value.length;
  if (j !== -1) return str.slice(0, j) + str.slice(jEnd, str.length);
  return str;
}
...
function countVowels(str) {
  ...
  for (let i = 0; i <= strArr.length - 1; i += 1) {
    for (let j = 0; j <= vowelsArr.length - 1; j += 1) {
...
function isPalindrome(str) {
  ...
  const reStrArr = strArr.split('').reverse().join('');
  return strArr === reStrArr;
}
...
function findLongestWord(sentence) {
  const srtArr = sentence.split(' ');
  let word = '';
  for (let i = 0; i < srtArr.length; i += 1) {
    if (srtArr[i].length > word.length) {
      word = srtArr[i];
    }
  }
  return word;
...
function reverseWords(str) {
  ...
  return srtArr.map((el) => el.split('').reverse().join('')).join(' ');
}
